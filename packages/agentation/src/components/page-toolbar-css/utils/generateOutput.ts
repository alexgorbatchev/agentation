...
export function generateOutput(
  ...
  _reactMode: ReactComponentMode = "filtered",
): string {
  ...
  annotations.forEach((annotation, index) => {
    if (detailLevel === "compact") {
      output += `${index + 1}. **${annotation.element}**${
        annotation.sourceFile ? ` (${annotation.sourceFile})` : ""
      }: ${annotation.comment}`;
    ...
    if (detailLevel === "forensic") {
      ...
      if (annotation.reactComponents) {
        output += `**React:** ${annotation.reactComponents}\n`;
      }
    ...
    if (detailLevel === "detailed") {
      ...
      if (annotation.boundingBox) {
        output +=
          `**Position:** ${Math.round(annotation.boundingBox.x)}px, ${Math.round(annotation.boundingBox.y)}px ` +
          `(${Math.round(annotation.boundingBox.width)}×${Math.round(annotation.boundingBox.height)}px)\n`;
      }
    }
